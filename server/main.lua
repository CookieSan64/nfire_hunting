...
if not lib.checkDependency('ox_lib', '3.16.2') then error('You don\'t have latest version of ox_lib') end
if not lib.checkDependency('ox_inventory', '2.38.1') then error('You don\'t have latest version of ox_inventory') end
if not lib.checkDependency('ox_target', '1.14.0') then error('You don\'t have latest version of qtarget') end
...
AddEventHandler('nfire_hunting:harvestCarcass',function (entityId, bone)
    ...
    if #(playerCoords - entityCoords)< 5 then
        if Antifarm(source, entityCoords) then
            ...
            TriggerClientEvent('ox_inventory:notify', source, {type = 'error', text = "Respecte la biodiversité, change d'endroit"})
        end
    else
        TriggerClientEvent('ox_inventory:notify', source, {type = 'error', text = "Vous êtes trop loin de la carcasse"})
    end
end)
